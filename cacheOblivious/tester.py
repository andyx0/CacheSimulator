...
def test_matTrans(n, path="./", verbose=False):
    ...
    try:
        ...
        if verbose:
            ...
            print("result")
            print(resultlist)
        assert resultlist == answer, "The matrix transpose result doesn't match answers/matrix_b_{}x{}.txt.".format(
            n, n)
        return True
...
def test_cacheOblivious(n, test_name, path="./", verbose=False):
    ...
    try:
        generate_trace(n=n, path=path, dut='./cacheOblivious',
                       trace_path="cacheOblivious_trace_{}.txt".format(test_name))
        csim = subprocess.run(
            ['../csim-ref', '-s', '2', '-E', '4', '-b', '4', '-l', '1',
        ...
        assert result_tallies[1] < answer_tallies[1] / \
            2, "Cache misses need to be less than half of baseline."
        assert result_tallies[2] < answer_tallies[2] / \
            2, "Cache evictions need to be less than half of baseline."
        return True
    except subprocess.CalledProcessError as e:
        print(e.output)
        print("Calling ./cacheOblivious returned non-zero exit status.")
...
def score_cacheOblivious(path='./', verbose=False):
...
if __name__ == '__main__':
    # generate_test_suite()
    score_cacheOblivious(verbose=True)
    exit()
