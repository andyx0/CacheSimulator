...
def test_fullyAssociative(test_name, path="./", verbose=False):
    ...
    try:
        ...
        if verbose:
            ...
            print("result")
            print(result.stdout)
        assert answer == result.stdout, "The printed result doesn't match answers/answer_{}.txt.".format(
            test_name)
        return True
...
def score_fullyAssociative(path='./', verbose=False):
...
if __name__ == '__main__':
    # generate_test_suite()
    score_fullyAssociative(verbose=True)
    exit()
