...
def test_matMul(n, path="./", verbose=False):
    ...
    try:
        result = subprocess.run(
            ['./cacheBlocking', "../matMul/tests/matrix_a_{}x{}.txt".format(
                n, n), "../matMul/tests/matrix_b_{}x{}.txt".format(n, n)],
            cwd=path,
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
        ...
        if verbose:
            ...
            print("result")
            print(resultlist)
        assert resultlist == answer, "The matrix multiplication result doesn't match answers/matrix_c_{}x{}.txt.".format(
            n, n)
        return True
...
def test_cacheBlocking(n, test_name, path="./", verbose=False):
    ...
    try:
        generate_trace(n=n, path=path, dut='./cacheBlocking',
                       trace_path="cacheBlocking_trace_{}.txt".format(test_name))
        csim = subprocess.run(
            ['../csim-ref', '-s', '2', '-E', '4', '-b', '4', '-l', '1',
...
def score_cacheBlocking(path='./', verbose=False):
...
if __name__ == '__main__':
    # generate_test_suite()
    score_cacheBlocking(verbose=True)
    exit()
